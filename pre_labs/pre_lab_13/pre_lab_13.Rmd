...
**Answer** The dataframe shows how states voted in the 2016 election, the percent of votes that went to Trump, the percent of small businesses that received PPP loans, and the average amount of money that the PPP provided for the employees of small businesses.
```{r}
...
**Answer** I see a scatterplot that compares the percent of voters that voted for trump and the percent of businesses that received ppp loans in the states. There is also a line showing the trend.
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer** This shows another scatterplot that has the percent of votes for trump and the amount of ppp per small business employees. It also has a line of best fit that shows the trend. This one seems harder to read in my opinion.
```{r}
...
**Answer** This is a dataframe that only shows the two variables that have to do with PPP data.
```{r}
reveal_data %>%
...
**Answer** This gives the t value, the degrees of freedom, p value, alternative hypothesis, and the confidence interval for the percent trump and business percent variables. It also gives the cor value.
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer** This compares the percent trump and ppp per employee variables and gives the same values as above: t, df, p-value, confidence interval, and correlation.
```{r}
...
**Answer** It shows the average amounts of ppp per employee and percent of businesses who received ppp in Clinton States and in Trump states. 
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer** It is the T test for ppp per employee by vote. T is around -1 and the p value is 0.2295. It also gives the confidence interval and the means for both groups.
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer** It is a t test for percent of businesses who received ppp by vote. The t value is -3, and the p value is 0.002643. This also shows the confidence intervals and the means for both groups. 
```{r}
